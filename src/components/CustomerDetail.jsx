...
const { FiUser, FiMail, FiPhone, FiBriefcase, FiMapPin, FiCalendar, FiDollarSign, FiTag, FiMessageSquare, FiEdit, FiPlus, FiUsers } = FiIcons;
      ...
      >
        <div className="p-6">
          ...
          <div className="flex border-b border-gray-200 mb-6">
            ...
            >
              컨택 이력 ({formatNumber(eventStats.total)})
            </button>
          </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        ...
                        ({formatDistanceToNow(new Date(customer.createdAt), { addSuffix: true, locale: ko })})
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  {customer.users && (
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{formatNumber(customer.users)}명</p>
                      </div>
                    </div>
                  )}
                  {customer.monthlyFee && (
                ...
                <div className="mt-6 bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-primary-600">{formatNumber(eventStats.meetings)}</p>
                      <p className="text-sm text-gray-500">미팅</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-blue-600">{formatNumber(eventStats.calls)}</p>
                      <p className="text-sm text-gray-500">통화</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-orange-600">{formatNumber(eventStats.followups)}</p>
                      <p className="text-sm text-gray-500">팔로업</p>
                    </div>
                  </div>
                  {eventStats.lastContact && (
                    <div className="mt-3 text-center text-sm text-gray-600">
                      마지막 컨택: {formatDistanceToNow(parseISO(eventStats.lastContact), { addSuffix: true, locale: ko })}
                    </div>
                  )}
                </div>
              </div>
